...
ipcMain.on('start-cube-video-stream', async (event) => {
  // 비디오 스트리밍을 시작
  startCubeVideoStream(event);
});

function startCubeVideoStream(event: Electron.IpcMainEvent) {
  const call = GrpcClient.VideoGrpcClient.streamCube();
  ...
  call.on('end', () => {
    console.log('Cube video stream ended');
  });

  call.on('error', (error) => {
    console.error('Cube video stream error:', error);
  });
}
